...
def upload(username):
    ...
    if request.method == "POST":
        ...
        if file_ru and file_zh:
            ...
            file_ru.save(raw_ru)
            file_zh.save(raw_zh)
            return render_template("upload.html", files=itertools.zip_longest(get_raw_files_list(username, RU_CODE), get_raw_files_list(username, ZH_CODE)), username = username)
    return render_template("upload.html", files=itertools.zip_longest(get_raw_files_list(username, RU_CODE), get_raw_files_list(username, ZH_CODE)), username = username)
